// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'timer_log.dart';

// **************************************************************************
// TypeAdapterGenerator
// **************************************************************************

class TimerLogAdapter extends TypeAdapter<TimerLog> {
  @override
  final int typeId = 4;

  @override
  TimerLog read(BinaryReader reader) {
    final numOfFields = reader.readByte();
    final fields = <int, dynamic>{
      for (int i = 0; i < numOfFields; i++) reader.readByte(): reader.read(),
    };
    return TimerLog(
      id: fields[0] as String,
      sagId: fields[1] as String,
      date: fields[2] as String,
      type: fields[3] as String,
      customType: fields[4] as String?,
      hours: fields[5] as double,
      rate: fields[6] as double,
      billable: fields[7] as bool,
      note: fields[8] as String?,
      user: fields[9] as String,
      timestamp: fields[10] as String,
    );
  }

  @override
  void write(BinaryWriter writer, TimerLog obj) {
    writer
      ..writeByte(11)
      ..writeByte(0)
      ..write(obj.id)
      ..writeByte(1)
      ..write(obj.sagId)
      ..writeByte(2)
      ..write(obj.date)
      ..writeByte(3)
      ..write(obj.type)
      ..writeByte(4)
      ..write(obj.customType)
      ..writeByte(5)
      ..write(obj.hours)
      ..writeByte(6)
      ..write(obj.rate)
      ..writeByte(7)
      ..write(obj.billable)
      ..writeByte(8)
      ..write(obj.note)
      ..writeByte(9)
      ..write(obj.user)
      ..writeByte(10)
      ..write(obj.timestamp);
  }

  @override
  int get hashCode => typeId.hashCode;

  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      other is TimerLogAdapter &&
          runtimeType == other.runtimeType &&
          typeId == other.typeId;
}
